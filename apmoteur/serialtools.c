    ...
    if(err_l = Laser_Init2Laser(&ml, &sl)){
        ...
        if(err_l = Laser_Start2Laser(&ml, &sl)){
            ...
            if(err_l == LASER_MASTER_START_ERROR){
                ...
                gui_image_set("imgLaserStateD", "gtk-no", 2);
                gui_label_set("labLaserStateG", LASER_STATUS_OK_LABEL);
                gui_label_set("labLaserStateG", SLAVE_NOT_STARTED_LABEL);
            }
        }
        if(!err_l){
            gui_image_set("imgLaserStateG", "gtk-yes", 2);
...
//affichage mesures non vérifiées
    ...
    gui_label_set("labLaserDataD", strtools_gnum2str_all_decimal((UNS32*)&mes1, uint32));

    Laser_GetUnverifiedData(&ml, &mes2);
    gui_label_set("labLaserDataG", strtools_gnum2str_all_decimal((UNS32*)&mes2, uint32));
...
*/
    ...
    gui_label_set("labLaserDataV", strtools_gnum2str_all_decimal((UNS32*)&(d.mes), uint32));
