...
void * Thread_Appli_Vitesse(void * vc)
{
  ...
  cantools_ApplyVelocity(v.cv,"Rotation");

  usleep(v.time);

  printf("VITESSE APPLIQUEE -> %d\n",v.dv );
  cantools_ApplyVelocity(v.dv,"Rotation");
...
int Find_Asserv_Params( long int * cp, INTEGER32 * p_capt_mrot, double * dvit_corr, INTEGER32 * v_old_mrot, struct Laser_x_thread * vc, int index_rot)
{
  double a  = (double)slave_get_param_in_num("Acceleration", index_rot);//tjr >0
  double d = (double)slave_get_param_in_num("Deceleration", index_rot);
...
int Find_Asserv_Params2( long int * cp, INTEGER32 * p_capt_mrot, INTEGER32 * v_old_mrot, struct Laser_x_thread * vc, int index_rot)
{
  double d_vit_corr = LASER_ASSERV_DELTAVITESSEMAX_CORRECTION;

  if(Find_Asserv_Params(cp, p_capt_mrot, &d_vit_corr, v_old_mrot, vc, index_rot) < 0){
    return -1;
  }
...
unsigned int CheckAndCorrect_VelocityRelation(unsigned long * d, INTEGER32 * v_cons_mt, INTEGER32 * v_cons_mrot)
{
  ...
  if((*v_cons_mrot) != v_cons_calc_mrot)
    {
      *v_cons_mrot = v_cons_calc_mrot;
      cantools_ApplyVelocity(v_cons_calc_mrot, "Rotation");
    }
...
unsigned int laser_asserv_User_Velocity_Change(laser * ml, laser * sl, INTEGER32 v_cons_mt)
{
  ...
  cantools_ApplyVelocity(v_cons_mrot, "Rotation");
  cantools_ApplyVelocity(v_cons_mt, "Translation");
...
unsigned int laser_asserv_Follow_Consigne(laser * ml, laser * sl)
{
  ...
  unsigned int res = 0;
    ...
    if(slave_get_indexList_with_ProfileName_with_Title("vitesse", "Rotation", &index_rot)){
        ...
        return LASER_ASSERV_READ_DAT_ERROR;
    }
  ...
  v_cons_mt = slave_get_param_in_num("Vel2send", index_trans);
  v_cons_mrot = slave_get_param_in_num("Velocity", index_rot);
...
//data moteur translation: "Status_word_MotTrans"
{
  ...
  *next_Sync_call_Nr += 1;
    ...
    //recupération des index intéressants
    int index_rot, index_trans;

    if(slave_get_indexList_from_ProfileName("RotationVitesse", &index_rot)){
        errgen_set(ERR_LASER_ASSERV_NOVELMOTROTDEFINED, NULL);
        return LASER_ASSERV_READ_DAT_ERROR;
    }
    if(slave_get_indexList_from_ProfileName("TranslationVitesse", &index_trans)){
        errgen_set(ERR_LASER_ASSERV_NOTRANSMOTDEFINED, NULL);
        return LASER_ASSERV_READ_DAT_ERROR;
    }

  v_capt_mrot = slave_get_param_in_num("Velocity", index_rot);
  p_capt_mrot = slave_get_param_in_num("Position", index_rot);
  v_cons_mrot = slave_get_param_in_num("Vel2send", index_rot);
  v_cons_mt = slave_get_param_in_num("Vel2send", index_trans);
	...
	case VITESSE_CONSTANTE:
	  {
	    ...
	    if(err < User_Tolerance)
	      {
		if(Find_Asserv_Params2(&cp, &p_capt_mrot, &v_cons_mrot/*entrees*/, &vc/*sortie*/, index_rot))
		  {
