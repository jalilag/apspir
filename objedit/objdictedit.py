...
if loc is None:
    ...
    __builtin__.__dict__['loc'] = loc,suffixe
...
class objdictedit(wx.Frame, NodeEditorTemplate):
    ...
    def __init__(self, parent, manager = None, filesOpen = []):
        global suffixe
        if manager is None:
            NodeEditorTemplate.__init__(self, NodeManager(suffixe), self, True, suffixe)
        else:
            NodeEditorTemplate.__init__(self, manager, self, False, suffixe)
        self._init_ctrls(parent)
