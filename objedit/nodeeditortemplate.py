...
class NodeEditorTemplate:
    ...
    def __init__(self, manager, frame, mode_solo, suffixe):
...
#-------------------------------------------------------------------------------
    ...
    def OnNodeInfosMenu(self, event):
        ...
        dialog.SetValues(name, id, type, description, defaultstringsize,suffixe)
        if dialog.ShowModal() == wx.ID_OK:
            name, id, type, description, defaultstringsize, suffixe = dialog.GetValues()
            self.Suffixe = suffixe
            self.Manager.Suffixe = suffixe
