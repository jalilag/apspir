...
def GenerateFileContent(Node, headerfilepath,suff, pointers_dict = {}):
    """
...
#-------------------------------------------------------------------------------
    ...
    for index in listIndex:
        ...
        if index in variablelist:
            strIndex += "\n/* index 0x%(index)04X :   Mapped variable %(EntryName)s%(add_id)s */\n"%texts
        else:
            strIndex += "\n/* index 0x%(index)04X :   %(EntryName)s%(add_id)s. */\n"%texts
        ...
        if not isinstance(values, ListType):
            ...
            if index in variablelist:
                ...
                texts["add_id"] = add_id
                strDeclareHeader += "extern %(subIndexType)s %(name)s%(add_id)s%(suffixe)s;\t\t/* Mapped at index 0x%(index)04X, subindex 0x00*/\n"%texts
                mappedVariableContent += "%(subIndexType)s %(name)s%(add_id)s%(suffixe)s = %(value)s;\t\t/* Mapped at index 0x%(index)04X, subindex 0x00 */\n"%texts
            else:
            ...
            if entry_infos["struct"] & OD_IdenticalSubindexes:
                ...
                if index in variablelist:
                    ...
                    strDeclareHeader += "extern %(subIndexType)s%(type_suffixe)s %(name)s%(add_id)s[%(values_count)s];\t\t/* Mapped at index 0x%(index)04X, subindex 0x01 - 0x%(length)02X */\n"%texts
                    mappedVariableContent += "%(subIndexType)s%(type_suffixe)s %(name)s%(add_id)s[] =\t\t/* Mapped at index 0x%(index)04X, subindex 0x01 - 0x%(length)02X */\n  {\n"%texts
                ...
                for subIndex, value in enumerate(values):
                    ...
                    if subIndex > 0:
                        ...
                        texts["add_id"] = add_id
                        if index in variablelist:
                            strDeclareHeader += "extern %(subIndexType)s %(parent)s_%(name)s%(add_id)s%(suffixe)s;\t\t/* Mapped at index 0x%(index)04X, subindex 0x%(subIndex)02X */\n"%texts
                            mappedVariableContent += "%(subIndexType)s %(parent)s_%(name)s%(add_id)s%(suffixe)s = %(value)s;\t\t/* Mapped at index 0x%(index)04X, subindex 0x%(subIndex)02X */\n"%texts
                        else:
        ...
        if callbacks:
            if index in variablelist:
                name = FormatName(entry_infos["name"]) + add_id
            else:
        ...
        for subIndex in xrange(len(values)):
            ...
            if subIndex == 0:
                ...
                if entry_infos["struct"] & OD_MultipleSubindexes:
                    ...
                    name = FormatName(subentry_infos["name"]) + add_id
                else:
                    name = FormatName("%s_obj%04X"%(texts["NodeName"], texts["index"]))
            elif entry_infos["struct"] & OD_IdenticalSubindexes:
                if index in variablelist:
                    name = "%s[%d]"%(FormatName(entry_infos["name"]), subIndex - 1) + add_id
                else:
                    name = "%s_obj%04X[%d]"%(texts["NodeName"], texts["index"], subIndex - 1)
            else:
                if index in variablelist:
                    name = FormatName("%s_%s"%(entry_infos["name"],subentry_infos["name"])) + add_id
                else:
...
def GenerateFile(filepath, node,suff,pointers_dict = {} ):
    try:
        headerfilepath = os.path.splitext(filepath)[0]+".h"
        content, header = GenerateFileContent(node, os.path.split(headerfilepath)[1],suff,pointers_dict)
